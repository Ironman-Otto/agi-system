...
1. **Separation of concerns** – semantic interpretation is isolated from execution logic
2. **Determinism** – downstream modules operate on structured fields, not text
3. **Extensibility** – new fields can be added without breaking existing logic
4. **Auditability** – every execution decision can be traced back to intent metadata
5. **Technology neutrality** – the schema does not assume a specific LLM or classifier
